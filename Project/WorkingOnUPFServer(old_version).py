...
#print(args)
train_dir = '/shared/home/u124298/data/models/train'#'train'#args.dir
threshold = 5

captions_dict = load_captions(train_dir)
vocab = Vocabulary(captions_dict, threshold)
#with open(os.path.join(vgg, 'vocab.pkl'), 'wb') as f:   #first argument args.model
#	pickle.dump(vocab, f)
...
optimizer = torch.optim.Adam(params, lr = 1e-5) #args.learning_rate
num_epochs = 101              ########################################
...
#	args = parser.parse_args()
'''
#with open(os.path.join(args.model, path/'vocab.pkl'), 'rb') as f:
with open('/content/drive/Shared drives/DANI IVAN EDGAR MARÇAL/DEEPLEARNING/Ivan - Marçal/Deep_Learning2020/Final Project/results/vocabulari/'+'vocab.pkl', 'rb') as f:
	vocab = pickle.load(f)
	print("load")
...
dev_dir = '/content/drive/Shared drives/DANI IVAN EDGAR MARÇAL/DEEPLEARNING/Ivan - Marçal/Deep_Learning2020/Final Project/data/flick8k/models/train'
#captions_dict = load_captions(dev_dir)
#vocab = Vocabulary(captions_dict, threshold)

dataloader = DataLoader(dev_dir, vocab, transform)
data = dataloader.gen_data()
print(results_path + ' loaded')
'''
...
for iteration in range(0, num_epochs, 10):
	#cnn_file = '/alexnetiter_0_cnn.pkl'         ##############################
	#lstm_file = '/alexnetiter_0_lstm.pkl'       ##############################
...
#with open(results_path+'/vocabulari/vocab.pkl', 'rb') as f:
#		vocab = pickle.load(f)
